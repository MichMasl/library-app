...
readonly class GrandAccessMiddleware implements MiddlewareInterface
{
    ...
    private function validateRequiredFields(array $data, ServerRequestInterface $request): void
    {
        if (!array_key_exists("userId", $data)) {
            throw new HttpBadRequestException($request, "userId is required");
        }
    }
    ...
    private function validateValues(array $data, ServerRequestInterface $request): void
    {
        if (empty($data['userId'])) {
            throw new HttpBadRequestException($request, "userId cannot be empty");
        }
